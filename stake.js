...
contract StakeTokens is Ownable, ReentrancyGuard {
	...
	bool private isActiveStake = false;
    ...
    function setPeriodFinish(uint256 number) public onlyOwner {
        periodFinish = number;
    }

	function setActiveStake(bool _isActive) public onlyOwner {
        isActiveStake = _isActive;
    }	

    function getTotalReward(address account) public view returns (uint256) {
        UserInfo storage user = userInfo[account];
        return user.amount * _reward;
    }
    ...
    function stake(uint256 _amount) external nonReentrant {
		...
		require(isActiveStake == true, "Cannot stake tokens");
    ...
    function withdraw() external nonReentrant {
        ...
        if (user.amount > 0) {  
            stakedToken.transfer(address(msg.sender), user.amount);            
